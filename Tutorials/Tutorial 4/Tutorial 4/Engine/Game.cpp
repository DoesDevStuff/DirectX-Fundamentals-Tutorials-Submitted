...
//initialising angle of rotation for teapot
float calc_newAngle = 0.f;
...
//	context->RSSetState(m_states->Wireframe());
	...
	float rotate_angle = 3.142 / 6;
	float angle_perFrame = rotate_angle / m_timer.GetFramesPerSecond();
	float calc_newAngle = angle_perFrame * m_timer.GetFrameCount();
	*/
	...
	SimpleMath::Matrix newRotation = SimpleMath::Matrix::CreateRotationY(calc_newAngle); // for rotatiing teapot (in radians)

	m_world = m_world * newRotation * newPosition * newRotation;// updating position in world with rotation
	...
	//prepare transform for second object. 
	m_world = SimpleMath::Matrix::Identity; //set world back to identity  [basically this is the identity matrix]
